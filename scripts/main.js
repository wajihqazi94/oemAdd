    ...
    let grabOEMProviders = function(api) {
		return new Promise(function(resolve, reject) {
			api.call("GetOEMCredentialsSchema", {
				"oemUrl":"https://oemregistry.geotab.com" 
			}, function(result) {
				resolve(result);
			}, function(e) {
				resolve(e);
    ...
    return {
        initialize: async function(api, state, initializeCallback) {
			...
			oemHelpBtn.addEventListener("click", async function() {
				setDialog("open", {
					title: "Help",
					content: "This page allows administrators to add OEM credentials to the MyGeotab database. For any fields that require multiple entries the values should be comma separated."
				});
			});
